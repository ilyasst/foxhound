...
update-time, and task-id tie-breakers. Pull-request review workflows are
selected first, email- and meeting-origin workflows next, and repository
issues last. Other non-issue sources share the email-and-meeting tier; within
each source tier, the explicit priority control and stable tie-breakers apply.
The acknowledgement contains only the
